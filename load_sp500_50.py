...
def sample_50(tickers, sectors, n=50):
...
def get_10k_text_edgar(ticker, max_retries=2):
    """
    Fetch the most recent 10-K filing's Item 1 (Business Description) from SEC EDGAR.
    Uses the EDGAR full-text search API to find filings, then extracts Item 1.
    """
    try:
        # Step 1: Get CIK for this ticker
        cik_url = f"https://efts.sec.gov/LATEST/search-index?q=%22{ticker}%22&forms=10-K&dateRange=custom&startdt=2022-01-01&enddt=2024-12-31"
        
        # Use simpler company search
        ...
        tickers_url = f"https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&company=&CIK={ticker}&type=10-K&dateb=&owner=include&count=1&search_text=&action=getcompany"
        
        # Actually, let's use the EDGAR company search API
        company_url = f"https://efts.sec.gov/LATEST/search-index?q=%22{ticker}%22&forms=10-K"
        
        # Best approach: Use SEC full-text search
        search_api = "https://efts.sec.gov/LATEST/search-index"
        
        # Let's use the EDGAR filing search
        resp = requests.get(
            f"https://efts.sec.gov/LATEST/search-index?q=%22{ticker}%22&forms=10-K&dateRange=custom&startdt=2022-01-01&enddt=2024-12-31",
...
def get_10k_text_edgar_v2(ticker, max_retries=2):
    """
    Alternative: Use EDGAR company filings API to get 10-K text.
    """
    try:
        # Get CIK from ticker
...
def main():
    print("=" * 60)
    print("Phase 3: 50 S&P 500 Stocks with 10-K Filing Analysis")
    print("=" * 60)
    
    # 1. Get tickers and sample 50
    all_tickers, all_sectors = get_sp500_tickers()
    selected, sel_sectors = sample_50(all_tickers, all_sectors, n=50)
    ...
    # 5. Save everything
    price_df.to_csv("data/raw/sp500_50_prices.csv")
    print(f"\nSaved: data/raw/sp500_50_prices.csv ({price_df.shape})")
    
    lex_df.to_csv("data/processed/lexical_matrix_50.csv")
    print(f"Saved: data/processed/lexical_matrix_50.csv ({lex_df.shape})")
    
    with open("data/processed/sector_map_50.json", "w") as f:
        json.dump(sel_sectors, f, indent=2)
    print(f"Saved: data/processed/sector_map_50.json ({len(sel_sectors)} tickers)")
    
    with open("data/raw/sp500_50_10k_texts.json", "w") as f:
        json.dump({
            "descriptions": descriptions,
            "sources": sources,
        }, f, indent=2)
    print(f"Saved: data/raw/sp500_50_10k_texts.json")
